define(['util/utils', 'util/codeUtil', 'util/domUtil'],
  ...
  Export.getUId = function() {
    return "area-multi-select-" + (++ctrlIndex);
  };
  ...
  Export.init = function() {
    ...
    return utils.loadTemplate(null, 'area-multi-select-template', 'widget/areaMultiSelect.tpl')
    .then(function(html) {
      me.template = html;
      ...
      if (me.left) {
        me.template.find(".arrow").css("left", me.left + "px");
      } else {
      ...
      var uid = me.getUId();
      me.activeTemplate = utils.loadTemplateLocal(null, 'area-multi-select-content-template', {uid:uid});
      me.inited = true;
    });
  };
   ...
   * @returns {Export.bind}
   */
  Export.bind = function($owner, getCallback, setcallback, level, itemcallback, left) {
    var newUtil = $.extend(true, {}, this);
    newUtil._bind($owner, getCallback, setcallback, level, itemcallback, left);
    $owner.data("areaMultiSelect", newUtil);
    return newUtil;
  };
  
  Export._bind = function($owner, getCallback, setcallback, level, itemcallback, left) {
    var me = this;
  ...
  Export.saveActiveTemplate = function() {
    ...
    this.activeTemplate = $popover.find(".area-multi-select-list");
  };
  ...
  Export.getArea = function(dataId, tabId) {
    ...
    .then(function(result) {
      ...
      data = result;
      return utils.loadTemplateLocal($popover.find('#' + tabId), 'area-multi-select-item-template', result||{});
    }).then(function() {
  ...
  Export.bindCellClick = function() {
    var me = this;
    var $popover = me.$owner.data("bs.popover").tip();
    $popover.find(".area-multi-select-list .tab-content a").off("click").on("click", function (e) {
  ...
  Export.bindEvent = function() {
    ...
    var $popover = me.$owner.data("bs.popover").tip();
    $popover.find(".area-multi-select-list .nav-tabs a").on("shown.bs.tab", function (e) {
      me.saveActiveTemplate();
    });
    
    $popover.find(".area-multi-select-list .nav-tabs a").on("click", function (e) {
      e.preventDefault();
      $(this).tab('show');
    });
    
    $popover.find("[name=confirm]").on("click", function(e) {
      me.saveValue();
      e.preventDefault();
    });
    
    $popover.find("[name=selectAll]").on("click", function(e) {
  ...
  Export.unbindEvent = function() {
    var me = this;
    var $popover = me.$owner.data("bs.popover").tip();
    $popover.find(".area-multi-select-list .nav-tabs a").off("shown.bs.tab");
    $popover.find(".area-multi-select-list .nav-tabs a").off("click");
    $popover.find(".area-multi-select-list .tab-content a").off("click");
    $popover.find("[name=confirm]").off("click");
    $popover.find("[name=selectAll]").off("click");
  };
