define(['util/utils', 'util/codeUtil', 'util/domUtil'],
  ...
  Export.init = function() {
    ...
    return utils.loadTemplate(null, 'area-select-template', 'widget/areaSelect.tpl')
    .then(function(html) {
      me.template = html;
      ...
      var uid = me.getUId();
      me.activeTemplate = utils.loadTemplateLocal(null, 'area-select-content-template', {uid:uid});
      me.inited = true;
    });
  };
  ...
  Export.saveActiveTemplate = function() {
    ...
    this.activeTemplate = $popover.find(".area-select-list");
  };
  ...
  Export.getArea = function(dataId, tabId) {
    ...
    .then(function(result) {
      ...
      data = result;
      return utils.loadTemplateLocal($popover.find('#' + tabId), 'area-select-item-template', result||{});
    }).then(function() {
  ...
  Export.bindCellClick = function() {
    var me = this;
    var $popover = me.$owner.data("bs.popover").tip();
    $popover.find(".area-select-list .tab-content a").off("click").on("click", function (e) {
  ...
  Export.saveValue = function() {
    ...
    me.saveActiveTemplate();
    if (me.setcallback) {
      me.setcallback(me.area, me.getAreaName());
    }
    if (!me.$parent) {
  ...
  Export.bindEvent = function() {
    ...
    var $popover = me.$owner.data("bs.popover").tip();
    $popover.find(".area-select-list .nav-tabs a").on("shown.bs.tab", function (e) {
      me.saveActiveTemplate();
    });
    $popover.find(".area-select-list .nav-tabs a").on("click", function (e) {
      e.preventDefault();
      $(this).tab('show');
    });
  };
  ...
  Export.unbindEvent = function() {
    var me = this;
    var $popover = me.$owner.data("bs.popover").tip();
    $popover.find(".area-select-list .nav-tabs a").off("shown.bs.tab");
    $popover.find(".area-select-list .nav-tabs a").off("click");
    $popover.find(".area-select-list .tab-content a").off("click");
  };
