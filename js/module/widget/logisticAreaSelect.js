define(['util/utils', 'util/codeUtil', 'util/domUtil'],
  ...
  Export.init = function() {
    ...
    return utils.loadTemplate(null, 'logistic-area-select-template', 'widget/logisticAreaSelect.tpl')
    .then(function(html) {
      me.template = html;
      ...
      var uid = me.getUId();
      me.activeTemplate = utils.loadTemplateLocal(null, 'logistic-area-select-content-template', {uid:uid});
      me.inited = true;
    });
  };
  ...
  Export.bind = function($owner, getCallback, setcallback, level, itemcallback) {
    ...
    $owner.data("logisticAreaSelect", newUtil);
    return newUtil;
  };
  ...
  Export.saveActiveTemplate = function() {
    ...
    this.activeTemplate = $popover.find(".logistic-area-select-list");
  };
  ...
  Export.getArea = function(dataId, tabId) {
    ...
    .then(function(result) {
      ...
      data = result;
      return utils.loadTemplateLocal($popover.find('#' + tabId), 'logistic-area-select-item-template', result||{});
    }).then(function() {
  ...
  Export.bindCellClick = function() {
    var me = this;
    var $popover = me.$owner.data("bs.popover").tip();
    $popover.find(".logistic-area-select-list .tab-content a").off("click").on("click", function (e) {
  ...
  Export.saveValue = function() {
    ...
    if (me.setcallback) {
      ...
      var deliveryPrice = $popover.find(".operation-footer input").val();
      ...
      me.setcallback({deliveryPrice: deliveryPrice, valArr: valArr});
    }
  ...
  Export.bindEvent = function() {
    ...
    var $popover = me.$owner.data("bs.popover").tip();
    $popover.find(".logistic-area-select-list .nav-tabs a").on("shown.bs.tab", function (e) {
      me.saveActiveTemplate();
    });
    
    $popover.find(".logistic-area-select-list .nav-tabs a").on("click", function (e) {
      e.preventDefault();
      $(this).tab('show');
    });
    
    $popover.find("[name=confirm]").on("click", function() {
      me.saveValue();
    });
    
    $popover.find("[name=selectAll]").on("click", function() {
  ...
  Export.unbindEvent = function() {
    var me = this;
    var $popover = me.$owner.data("bs.popover").tip();
    $popover.find(".logistic-area-select-list .nav-tabs a").off("shown.bs.tab");
    $popover.find(".logistic-area-select-list .nav-tabs a").off("click");
    $popover.find(".logistic-area-select-list .tab-content a").off("click");
