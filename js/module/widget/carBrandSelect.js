define(['util/utils', 'util/carBrandUtil', 'util/domUtil'],
  ...
  Export.getUId = function() {
    return "carbrand-select-" + (++ctrlIndex);
  };
  ...
  Export.init = function() {
    ...
    return utils.loadTemplate(null, 'carbrand-select-template', 'widget/carBrandSelect.tpl')
    .then(function(html) {
      me.template = html;
      // 因为bootstrap的tab直接使用id做关联，无法实现同页面多个控件情况，所以加上命名空间
      var uid = me.getUId();
      me.activeTemplate = utils.loadTemplateLocal(null, 'carbrand-select-content-template', {uid:uid});
      me.inited = true;
    });
  };
   ...
   * @returns
   */
  Export.getCarbrandName = function() {
    var carbrandName = [];
    if (this.carbrand.carBrandName) {
      carbrandName.push(this.carbrand.carBrandName);
      if (this.carbrand.carSeriesName) {
        carbrandName.push(this.carbrand.carSeriesName);
        if (this.carbrand.carModelName) {
          carbrandName.push(this.carbrand.carModelName);
        }
      }
    }
  ...
  Export.saveActiveTemplate = function() {
    var $popover = this.$owner.data("bs.popover").tip();
    this.activeTemplate = $popover.find(".carbrand-select-list");
  };
  ...
  Export.getCarbrand = function(dataId, tabId) {
    ...
    if (tabId.indexOf("carBrand") > -1) {
      ...
      def = carBrandUtil.getCarSeries(dataId);
    ...
    return def.then(function(result) {
      ...
      if (tabId.indexOf("carSeries") > -1) {
        for (var k in result) {
          result = {
              items: result[k]
          };
        }
      } else if (!result.items && $.isArray(result)) {
        result = {
          items: result
        };
      }
      
      utils.loadTemplateLocal($popover.find('#' + tabId), 'carbrand-select-item-template', result || {});
  ...
  Export.bindCellClick = function() {
    var me = this;
    var $popover = me.$owner.data("bs.popover").tip();
    $popover.find(".carbrand-select-list .tab-content a").off("click").on("click", function (e) {
      ...
      if (level === me.setting.level) {
        ...
        if (me.setting.itemcallback) {
          me.setting.itemcallback(me.carbrand, me.getCarbrandName());
        }
        me.saveValue();
        return;
      }
  ...
  Export.saveValue = function() {
    ...
    if (me.setting.setcallback) {
      if (me.setting.setcallback(me.carbrand, me.getCarbrandName()) === false) {
        return;
      }
    }
  ...
  Export.bindEvent = function() {
    ...
    var $popover = me.$owner.data("bs.popover").tip();
    $popover.find(".carbrand-select-list .nav-tabs a").on("shown.bs.tab", function (e) {
      me.saveActiveTemplate();
    });
    
    $popover.find(".carbrand-select-list .nav-tabs a").on("click", function (e) {
      e.preventDefault();
      $(this).tab('show');
    });
    
    var $dom = domUtil.getDomByName($popover.find(".carbrand-select-list-footer"));
  ...
  Export.unbindEvent = function() {
    var me = this;
    var $popover = me.$owner.data("bs.popover").tip();
    $popover.find(".carbrand-select-list .nav-tabs a").off("shown.bs.tab");
    $popover.find(".carbrand-select-list .nav-tabs a").off("click");
    $popover.find(".carbrand-select-list .tab-content a").off("click");
    
    var $footer = $popover.find(".carbrand-select-list-footer");
    if ($footer.length > 0) {
